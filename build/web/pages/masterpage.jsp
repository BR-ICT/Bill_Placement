...
<section class="container-w100">
    <div class="wrap-login100" style="width: 100%;margin-bottom: 0px;">
        ...
        <br>
        <label for="txtPeriod">Company(10 DEFAULT):</label>
        <input type="text" id="vCompanyInput" name="company" style="width: 70px">
        <label for="txtPeriod">Customer Code:</label>
        ...
        <br>
        <label for="txtPeriod">วิธีคิดวันInvoice</label>
        ...
        <label for="txtPeriod" id="vInvoicelab">จบ Invoiceวันที่</label>
        <input type="text" id="vInvoiceA" name="InvoiceA" style="width: 70px">
        <br>
        <label for="txtPeriod">วิธีคิดวันวางบิล</label>
        ...
        <label for="txtPeriod" id="vBilInput1lab">ที่</label>
        ...
        <br>
        <label for="txtPeriod"> วิธีคิดวันชำระ</label>
        <select style="width: 550px" name="type"  id="vPayInput">
            <option value="" selected="selected">ปกติ</option>
            <option value="1">วัน A ของสัปดาห์ที่ B</option>
            <option value="3">วัน A ของทุกเดือน(ใช้ของรอบ 7 วัน)</option>
            <option value="4">วัน A ของทุกเดือน(ใช้ของรอบ 15,30 วัน)</option>
            <option value="5">สามารถตกวันที่ A กับ B เท่านั้น</option>
            <option value="6">หากตกวันที่A จะตกวันที่ B หากตกวันที่ C จะเป็นวันที่ D (หลักๆใช้สำหรับ 15 วันที่มี 2 รอบต่อเดือน)</option>
            <option value="8">วันที่ A กับสิ้นเดือน</option>
                ...
                $.ajax({
                    ...
                    response = $.grep(response, function (item) {
                        return(!filter.RCOMPANY || (item.RCOMPANY.indexOf(filter.RCOMPANY) > -1))
                                ...
                                && (!filter.ROUNDPAYSPECIAL || (item.ROUNDPAYSPECIAL.indexOf(filter.ROUNDPAYSPECIAL) > -1));
...
//                alert(item.RDTOTA_KGS);
                ...
                formData.roundpayspecial = item.ROUNDPAYSPECIAL;
...
//                alert(item.RDTOTA_KGS);
        ...
        fields: [
            ...
            {title: "รอบชำระSPECIAL", name: "ROUNDPAYSPECIAL", css: "lvgb", type: "text", editing: true, align: "right", width: 50}
        ]
...
//            $("#vBilInput2lab").show();
    ...
    $("#vPayInput").change(function paychange()
    {
        ...
        if (payinput === '') {
            ...
            $("#vColInput1lab").show();
            $("#vColInput1").show();
            $("#vColInput2lab").text('จะตกเป็นวันที่');
            $("#vColInput2lab").show();
            $("#vColInput2").show();
            $("#vColInput3lab").text('ของเดือนถัดไปและ');
            $("#vColInput3lab").show();
            $("#vColInput3").show();
            $("#vColInput4lab").text('จะตกเป็นวันที่ของเดือนเดียวกัน');
            $("#vColInput4lab").show();
            $("#vColInput4").show();
        } else if (payinput === '8') {
            $("#vColInput1lab").text('วันที่');
