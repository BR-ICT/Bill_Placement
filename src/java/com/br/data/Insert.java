...
public class Insert {
    ...
    public static String addStartMonth(String cono, String divi, String year, String month, String invround, String customer, String startdate) throws Exception {
        //USE TO SHOW RESPOND USER WILL SEE
        String respond = "";
...
//        }
        try {
            if (conn != null) {
                ...
                //Check if the data is generated or not for this customer
                ...
                //Check to see if customer ID existed or not
                ...
                if (check.equals("0") && check1.equals("0")) {
                    while ((Integer.parseInt(currentmonth) <= Integer.parseInt(maxmonth) || Integer.parseInt(currentyear) != Integer.parseInt(maxyear)) && Integer.parseInt(currentyear) <= Integer.parseInt(maxyear)) {
                        if (firsttime) {
                            //Insert the first time of the loop
                            Statement stmt = conn.createStatement();
                            String step1 = "INSERT INTO BRLDTA0100.BP_STDATE(BPS_CONO,BPS_DIVI,BPS_CUNO,BPS_STDT)\n"
                                    + "SELECT BPM_CONO,BPM_DIVI,BPM_CUNO,'" + date2 + "'\n"
                            ...
                            //Insert after the first loop
                            Statement stmt = conn.createStatement();
                            String step1 = "INSERT INTO BRLDTA0100.BP_STDATE(BPS_CONO,BPS_DIVI,BPS_CUNO,BPS_STDT,BPS_RD,BPS_STS)\n"
                                    + "SELECT DISTINCT A.BPS_CONO,A.BPS_DIVI,A.BPS_CUNO, YEAR(CHAR(DATE(TIMESTAMP_FORMAT(cast(A.BPS_FNDT as varchar(8)), 'YYYYMMDD'))+ 1 DAYS )) * 10000\n"
...
//                    UPDATE END DATE OF START DATE TABLE USING MASTER TABLE 7 days
                            if (invround.equals("7")) {
                                query2 = "UPDATE BRLDTA0100.BP_STDATE C\n"
                                        ...
                                        + "AND YEAR (DATE(TIMESTAMP_FORMAT(cast(C.BPS_STDT as varchar(8)), 'YYYYMMDD'))) = '" + currentmonth + "'";
                            }
                            //15 days case
                            if (invround.equals("15")) {
                                query2 = "UPDATE BRLDTA0100.BP_STDATE C\n"
                                        ...
                                        + "END\n"
                                        + "FROM BRLDTA0100.BP_STDATE A,BRLDTA0100.BP_MASTER B\n"
                                        + "WHERE A.BPS_CONO = B.BPM_CONO AND A.BPS_CUNO = B.BPM_CUNO AND A.BPS_DIVI = B.BPM_DIVI\n"
                                        + "AND B.BPM_RINV = 15 AND C.BPS_CONO = A.BPS_CONO AND C.BPS_DIVI = A.BPS_DIVI  AND c.BPS_CUNO = A.BPS_CUNO AND A.BPS_STDT = C.BPS_STDT\n"
                                        + "AND B.BPM_CASEINV = 0\n"
                                        + "AND B.BPM_CONO = " + cono + "\n"
                                        + "AND B.BPM_DIVI =" + divi + " \n"
                                        + "AND A.BPS_CUNO = '" + customer + "')\n"
...
//30 days case
                            ...
                            querycase1 = "UPDATE BRLDTA0100.BP_STDATE C\n"
                                    ...
                                    + "AND A.BPS_DIVI =" + divi + "\n"
                                    + "AND B.BPM_CUNO ='" + customer + "'\n"
                                    + "AND C.BPS_CONO = A.BPS_CONO AND c.BPS_CUNO = A.BPS_CUNO AND A.BPS_STDT = C.BPS_STDT AND A.BPS_DIVI = C.BPS_DIVI)\n"
                            ...
                            query3 = "UPDATE BRLDTA0100.BP_STDATE C\n"
                                    ...
                                    + "AND B.BPM_DIVI = A.BPS_DIVI AND B.BPM_DIVI = A.BPS_DIVI \n"
                                    + "AND C.BPS_CONO = A.BPS_CONO \n"
                                    + "AND C.BPS_DIVI = A.BPS_DIVI\n"
                                    + "AND C.BPS_CUNO = A.BPS_CUNO\n"
                            ...
                            query4 = "UPDATE BRLDTA0100.BP_STDATE T1\n"
                                    ...
                                    + "WHERE T3.BPM_CONO  = T2.BPS_CONO\n"
                                    + "AND T3.BPM_CUNO = T2.BPS_CUNO\n"
                                    + "AND T3.BPM_DIVI = T2.BPS_DIVI\n"
                                    + "AND T2.BPS_CUNO= T1.BPS_CUNO\n"
                                    + "AND T2.BPS_CONO = T1.BPS_CONO\n"
                                    + "AND T2.BPS_DIVI = T1.BPS_DIVI\n"
...
//                            stmt.execute(query1);
                            query1 = "INSERT INTO BRLDTA0100.BP_STDATE(BPS_CONO,BPS_DIVI,BPS_CUNO,BPS_STDT,BPS_STS)\n"
                                    + "SELECT DISTINCT A.BPS_CONO,A.BPS_DIVI,A.BPS_CUNO, YEAR(CHAR(DATE(TIMESTAMP_FORMAT(cast(A.BPS_FNDT as varchar(8)), 'YYYYMMDD'))+ 1 DAYS )) * 10000\n"
                                    + "+ MONTH(CHAR(DATE(TIMESTAMP_FORMAT(cast(A.BPS_FNDT as varchar(8)), 'YYYYMMDD') + 1 DAY))) * 100\n"
                                    + "+ DAY(CHAR(DATE(TIMESTAMP_FORMAT(cast(A.BPS_FNDT as varchar(8)), 'YYYYMMDD') + 1 DAY)))  AS ENDDATE,'00'\n"
                                    + "FROM BRLDTA0100.BP_STDATE A\n"
                                    + "JOIN BRLDTA0100.BP_MASTER B ON A.BPS_CONO = B.BPM_CONO AND A.BPS_CUNO  = B.BPM_CUNO AND A.BPS_DIVI = B.BPM_DIVI\n"
            ...
            if (conn != null) {
                ...
                if (check.equals("0") && check1.equals("0")) {
                    while ((Integer.parseInt(month) <= Integer.parseInt(maxmonth) || Integer.parseInt(year) != Integer.parseInt(maxyear)) && Integer.parseInt(year) <= Integer.parseInt(maxyear)) {
                        ...
                        query1 = "INSERT INTO BRLDTA0100.BP_HBILL(BPH_CONO,BPH_DIVI,BPH_CUNO,BPH_STDT,BPH_FNDT,BPH_STS)\n"
                                + "SELECT A.BPS_CONO,A.BPS_DIVI,A.BPS_CUNO,A.BPS_STDT,A.BPS_FNDT,A.BPS_STS\n"
                                + "FROM BRLDTA0100.BP_STDATE A,BRLDTA0100.BP_MASTER B\n"
                                + "WHERE B.BPM_CUNO = A.BPS_CUNO\n"
                                + "AND B.BPM_CONO = A.BPS_CONO\n"
                                + "AND B.BPM_DIVI = A.BPS_DIVI\n"
...
//                           Create billdate case 1
                            case "1":
                                query6 = "UPDATE BRLDTA0100.BP_HBILL C\n"
                                        ...
                                        + "AND C.BPH_CUNO = A.BPH_CUNO AND A.BPH_CONO = C.BPH_CONO AND A.BPH_DIVI = C.BPH_DIVI AND A.BPH_STDT = C.BPH_STDT AND A.BPH_FNDT = C.BPH_FNDT\n"
                                        + "AND B.BPM_CASEBIL = '1'\n"
                                        + "AND A.BPH_CUNO = '" + customer + "'\n"
                                        + "AND B.BPM_CONO = " + cono + "\n"
                                        + "AND B.BPM_DIVI = " + divi + "\n"
                        ...
                        switch (caseforpay) {
                            case "0":
                                ...
                                querypay0 = "UPDATE BRLDTA0100.BP_HBILL C\n"
                                        ...
                                        + "AND A.BPH_STDT = C.BPH_STDT\n"
                                        + "AND A.BPH_FNDT = C.BPH_FNDT)\n"
                                        + " WHERE  EXISTS (SELECT\n"
                                        + "YEAR(CHAR(DATE(TIMESTAMP_FORMAT(cast( C.BPH_BPDT as varchar(8)), 'YYYYMMDD'))+ B.BPM_RCOL DAYS )) * 10000\n"
                                        + "+ MONTH(CHAR(DATE(TIMESTAMP_FORMAT(cast( C.BPH_BPDT as varchar(8)), 'YYYYMMDD'))+ B.BPM_RCOL DAYS)) * 100\n"
                                        + "+ DAY(CHAR(DATE(TIMESTAMP_FORMAT(cast(C.BPH_BPDT as varchar(8)), 'YYYYMMDD'))+ B.BPM_RCOL DAYS))\n"
...
//paycase 10
    ...
    public static String importInvoiceData(String month, String year, String invround, String cono, String divi) throws Exception {
        ...
        String query3 = "";
        String query4 = "";
        String querycheckmonth = "";
        String querycase1 = "";
        String querycase2 = "";
        //USE TO SHOW RESPOND USER WILL SEE
        String respond = "";
        //CHECK USE TO CHECK IF EVERYTHING IS RIGHT IF 1 MEANT SOMETHING WRONG
        String check = "0";
        String nextmonth = Integer.toString(parseInt(month) + 1);
        if ("13".equals(nextmonth)) {
            nextmonth = "1";
        ...
        try {
            if (conn != null) {
                ...
                Statement stmt3 = conn.createStatement();
                Statement stmt4 = conn.createStatement();
                Statement statementCheckMonth = conn.createStatement();
                Statement stmtcase1 = conn.createStatement();
                Statement stmtcase2 = conn.createStatement();
                //USE TO CHECKING MONTH OF THE CURRENT WEEK
                querycheckmonth = "SELECT DISTINCT (MONTH(DATE(TIMESTAMP_FORMAT(cast(A.BPS_STDT as varchar(8)), 'YYYYMMDD'))))\n"
                        + "FROM BRLDTA0100.BP_STDATE A,BRLDTA0100.BP_MASTER B\n"
                        + "Where YEAR(DATE(TIMESTAMP_FORMAT(cast(A.BPS_STDT as varchar(8)), 'YYYYMMDD'))) = " + year + "\n"
                        + "AND A.BPS_CONO = B.BPM_CONO \n"
                        + "AND A.BPS_CUNO = B.BPM_CUNO\n"
                        ...
                        + "AND A.BPS_DIVI  =" + divi + "\n"
                        + "AND B.BPM_RINV =" + invround;
                ResultSet mRes = statementCheckMonth.executeQuery(querycheckmonth);
                System.out.println("CHECKMONTH IF ITS ALREADY GENERATE\n" + querycheckmonth);
                //CHECKING IF MONTH ALREADY GENERATED
                while (mRes.next()) {
                ...
                // CHECK is 0 MENAT NO ERROR OR THE MONTH CAN GENERATE
                if ("0".equals(check)) {
                    //THIS IS LOOPING TO FINISH THE WAY TO CALCULATE EACH MONTH,WE LOOP 6 TIMES TO MAKE SURE WE DIDNT LEFT ANY SUPPLIER OUT WITHOUT FINISH THE STEP NEED TO CALCULATE THE DATE
                    for (int i = 1; i <= 6; i++) {
                        if (i == 1) {
                            //FIRST STEP WE INSERT EMPTY START DATE SO WE CAN START SOMEWHERE IT SELECT AND INSERT ONLY THE ONE THAT WILL BE IN NEXT MONTH AFTER THE LAST STEP LAST MONTH
                            query1 = "INSERT INTO BRLDTA0100.BP_STDATE(BPS_CONO,BPS_DIVI,BPS_CUNO,BPS_STDT,BPS_RD,BPS_STS)\n"
                                    + "SELECT DISTINCT A.BPS_CONO,A.BPS_DIVI,A.BPS_CUNO, YEAR(CHAR(DATE(TIMESTAMP_FORMAT(cast(A.BPS_FNDT as varchar(8)), 'YYYYMMDD'))+ 1 DAYS )) * 10000\n"
                                    + "+ MONTH(CHAR(DATE(TIMESTAMP_FORMAT(cast(A.BPS_FNDT as varchar(8)), 'YYYYMMDD') + 1 DAY))) * 100\n"
                                    + "+ DAY(CHAR(DATE(TIMESTAMP_FORMAT(cast(A.BPS_FNDT as varchar(8)), 'YYYYMMDD') + 1 DAY)))  AS ENDDATE,'1','00'\n"
                                    + "FROM BRLDTA0100.BP_STDATE A\n"
                                    ...
                                    + "AND B.BPM_DIVI = " + divi + "\n"
                                    + "AND B.BPM_RINV =" + invround;
                            System.out.println("First step start date month empty insert\n" + query1);
                        } else {
                            query1 = "INSERT INTO BRLDTA0100.BP_STDATE(BPS_CONO,BPS_DIVI,BPS_CUNO,BPS_STDT,BPS_STS)\n"
                                    + "SELECT DISTINCT A.BPS_CONO,A.BPS_DIVI,A.BPS_CUNO, YEAR(CHAR(DATE(TIMESTAMP_FORMAT(cast(A.BPS_FNDT as varchar(8)), 'YYYYMMDD'))+ 1 DAYS )) * 10000\n"
                                    + "+ MONTH(CHAR(DATE(TIMESTAMP_FORMAT(cast(A.BPS_FNDT as varchar(8)), 'YYYYMMDD') + 1 DAY))) * 100\n"
                                    ...
                                    + "JOIN BRLDTA0100.BP_MASTER B ON A.BPS_CONO = B.BPM_CONO AND A.BPS_CUNO  = B.BPM_CUNO AND A.BPS_DIVI  = B.BPM_DIVI\n"
                                    + "WHERE MONTH(DATE(TIMESTAMP_FORMAT(cast(A.BPS_FNDT as varchar(8)), 'YYYYMMDD'))+ 1 DAY) = MONTH(DATE(TIMESTAMP_FORMAT(cast(A.BPS_STDT as varchar(8)), 'YYYYMMDD')))\n"
                                    + "AND BPS_RD = (SELECT MAX(C.BPS_RD) FROM BRLDTA0100.BP_STDATE C,BRLDTA0100.BP_MASTER D WHERE MONTH(DATE(TIMESTAMP_FORMAT(cast(c.BPS_STDT as varchar(8)), 'YYYYMMDD'))) = 7\n"
                                    + "AND YEAR (DATE(TIMESTAMP_FORMAT(cast(C.BPS_STDT as varchar(8)), 'YYYYMMDD'))) = 2025  AND D.BPM_CONO = C.BPS_CONO AND D.BPM_CUNO = C.BPS_CUNO AND D.BPM_DIVI = c.BPS_DIVI\n"
                                    + "AND D.BPM_CONO = " + cono + "\n"
                                    + "AND D.BPM_DIVI = " + divi + "\n"
                                    ...
                                    + "AND B.BPM_DIVI = " + divi + "\n"
                                    + "AND B.BPM_RINV =" + invround;
                            System.out.println("StartDate Month after first step insert \n" + query1);
                        }
//                    UPDATE END DATE OF START DATE TABLE USING MASTER TABLE 7 days
                        if (invround.equals("7")) {
                            query2 = "UPDATE BRLDTA0100.BP_STDATE C\n"
                            ...
                            System.out.println("Insert EndDate 7 \n" + query2);
                        }
                        //15 days case
                        if (invround.equals("15")) {
                            query2 = "UPDATE BRLDTA0100.BP_STDATE C\n"
                            ...
                            System.out.println("Insert EndDate 15 \n" + query2);
...
//30 days case
                        if (invround.equals("30")) {
                            query2 = "UPDATE BRLDTA0100.BP_STDATE C\n"
                                    ...
                                    + "AND SUBSTRING(C.BPS_STDT,5,2) =" + nextmonth + "\n"
                                    + "AND SUBSTRING(C.BPS_STDT,1,4) =" + year;
                            System.out.println("Insert EndDate 30 \n" + query2);
                        }
                        //QUERY CASE 1 FOR GENERATE END DATE OF THE CASE 1
                        querycase1 = "UPDATE BRLDTA0100.BP_STDATE C\n"
                                + "SET C.BPS_FNDT = (SELECT SUBSTRING(B.BPM_CASEINV,3,2) + \n"
                                + "MONTH(DATE(TIMESTAMP_FORMAT(cast(A.BPS_STDT as varchar(8)), 'YYYYMMDD'))+1 month) * 100\n"
                                + "+ YEAR (DATE(TIMESTAMP_FORMAT(cast(A.BPS_STDT as varchar(8)), 'YYYYMMDD'))+1 month) * 10000\n"
                                ...
                                + "AND SUBSTRING(C.BPS_STDT,5,2) = " + nextmonth + "\n"
                                + "AND SUBSTRING(C.BPS_STDT,1,4) =" + year;
                        System.out.println("Insert EndDate Case1 \n" + querycase1);
                        //QUERY CASE 2 FOR GENERATE END DATE OF THE CASE 2
                        querycase2 = "UPDATE BRLDTA0100.BP_STDATE C\n"
                                + "SET C.BPS_FNDT = (SELECT \n"
                                + "YEAR (DATE(DAYS(DATE(SUBSTRING(A.BPS_STDT,1,4)||'-'||SUBSTRING(A.BPS_STDT ,5,2)||'-01') + 2 MONTH) - 2)) * 10000\n"
                                + "	+ MONTH(DATE(DAYS(DATE(SUBSTRING(A.BPS_STDT,1,4)||'-'||SUBSTRING(A.BPS_STDT ,5,2)||'-01') + 2 MONTH) - 2)) * 100\n"
                                ...
                                + "AND SUBSTRING(C.BPS_STDT,5,2) =" + nextmonth + "\n"
                                + "AND SUBSTRING(C.BPS_STDT,1,4) =" + year;
                        System.out.println("Insert EndDate Case2 \n" + querycase2);
                        //Create period
                        query3 = "UPDATE BRLDTA0100.BP_STDATE C\n"
                                + "SET BPS_PR = (SELECT SUBSTRING(A.BPS_STDT,3,8) || SUBSTRING(A.BPS_FNDT,3,8)\n"
                                + "FROM BRLDTA0100.BP_STDATE A,BRLDTA0100.BP_MASTER B\n"
    ...
    public static String importStartData(String cono, String divi, String month, String year, String invround) throws Exception {
        ...
        try {
            if (conn != null) {
                ...
                Statement stmtcasepay10 = conn.createStatement();
                Statement stmtskipsundaypay = conn.createStatement();
                //Check if data generated yet or not
                ...
                //check if the start date existed
                query5 = "SELECT DISTINCT (MONTH(DATE(TIMESTAMP_FORMAT(cast(A.BPS_STDT as varchar(8)), 'YYYYMMDD'))))\n"
                        + "FROM BRLDTA0100.BP_STDATE A,BRLDTA0100.BP_MASTER B\n"
                        + "Where YEAR(DATE(TIMESTAMP_FORMAT(cast(A.BPS_STDT as varchar(8)), 'YYYYMMDD'))) = 2025\n"
                        + "AND A.BPS_CONO = B.BPM_CONO \n"
                        + "AND A.BPS_CUNO = B.BPM_CUNO\n"
                        + "AND A.BPS_DIVI  = B.BPM_DIVI\n"
                        + "AND B.BPM_RINV = " + invround + "\n"
                ...
                if (check.equals("0") && check1.equals("0")) {
                    ...
                    querypaycase3 = "UPDATE BRLDTA0100.BP_HBILL C\n"
                            ...
                            + "END\n"
                            + "FROM BRLDTA0100.BP_HBILL A, BRLDTA0100.BP_MASTER B\n"
                            + "WHERE A.BPH_CONO = B.BPM_CONO AND A.BPH_CUNO = B.BPM_CUNO AND A.BPH_DIVI = B.BPM_DIVI\n"
                            + "AND B.BPM_CASECOL = '3'\n"
                            + "AND B.BPM_CONO = " + cono + "\n"
                            + "AND B.BPM_DIVI = " + divi + "\n"
...
//case for paycase 4
    ...
    public static String insertfinanceheader(String company, String divi, String customerid, String startdate, String month, String year, String status, String invround) throws Exception {
        ...
        try {
            if (conn != null) {
                ...
                for (int i = 1; i <= 6; i++) {
                    if (i == 1) {
                        query1 = "INSERT INTO BRLDTA0100.BP_STDATE(BPS_CONO,BPS_CUNO,BPS_STDT,BPS_RD,BPS_STS,BPS_DIVI)\n"
                                + "VALUES ('" + company + "', '" + customerid + "','" + startdate + "','1','00'," + divi + ")";
                        System.out.println("InsertFacility\n" + query1);
                    } else {
